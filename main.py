...
def generate_unique_headline(original_title, api_key):
    client = Groq(api_key=api_key)
    try:
        prompt = f"Rewrite this news headline to be catchy and SEO-friendly in English: {original_title}. Return ONLY the new headline."
...
def generate_ai_content(headline, image_url, api_key):
    ...
    prompt = f"Topic: {headline}\nTask: Write a detailed, 800-word professional news article in English. Structure: Introduction, Analysis, Conclusion. Format: Use HTML (<h2>, <p>, <b>). Strictly English."
    try:
        completion = client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[{"role": "system", "content": "You are a senior news editor."}, {"role": "user", "content": prompt}],
            max_tokens=3000
        )
...
def post_to_blogger(service, title, content):
    try:
        service.posts().insert(blogId=BLOG_ID, body={"kind": "blogger#post", "title": title, "content": content}).execute()
        return True
    except: return False
...
def main():
    ...
    # Find the FIRST news that is NOT in our posted list
    news_to_post = None
    ...
    if not news_to_post:
        print("DEBUG: Everything in feed is already posted.")
        return

    print(f"DEBUG: Selected New Article: {news_to_post.title}")
    api_key = get_current_key()
    headline = generate_unique_headline(news_to_post.title, api_key)
    ...
    # In GitHub Actions, we exit after one successful post to sync state
